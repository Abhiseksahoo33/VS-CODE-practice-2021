...
namespace EmployeeDatabaseDemo
{
    class Program
    {
        ...
        {
            SqlConnection conn = new SqlConnection(@"integrated security=true ; data source=ABHI-SEK\MSSQLSERVER01;initial catalog=sqlpractice");
            // @ is add to accept the / symbol in the server name 
            SqlDataAdapter ad = new SqlDataAdapter("select * from Emp", conn);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Emp");
            foreach (DataRow dr in ds.Tables["Emp"].Rows)
            {
                Console.WriteLine("Employee No         :  " + dr["Empno"]);
                Console.WriteLine("         Name       :  " + dr["name"]);
                Console.WriteLine("         Department :  " + dr["dept"]);
                Console.WriteLine("         degignation: " + dr["desig"]);
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                Console.WriteLine("5: Show All Employee details");
